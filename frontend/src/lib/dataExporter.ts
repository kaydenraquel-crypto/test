...
export class DataExporter {
  ...
  private static generateCSV(data: ExportData): string {
    ...
    if (data.indicators) {
      ...
      if (data.indicators.sma && data.indicators.sma.length > 0) {
        ...
        data.indicators.sma.forEach((item: any) => {
      ...
      if (data.indicators.ema && data.indicators.ema.length > 0) {
        ...
        data.indicators.ema.forEach((item: any) => {
          const date = new Date(item.time * 1000).toISOString()
      ...
      if (data.indicators.rsi && data.indicators.rsi.length > 0) {
        ...
        data.indicators.rsi.forEach((item: any) => {
          const date = new Date(item.time * 1000).toISOString()
      ...
      if (data.indicators.bollinger && data.indicators.bollinger.length > 0) {
        ...
        data.indicators.bollinger.forEach((item: any) => {
          const date = new Date(item.time * 1000).toISOString()
  ...
  private static generateSummarySection(data: ExportData): string {
    ...
    return `
      <div class="section">
        ...
        <div class="summary-stats">
          ...
          <div class="stat-box">
            <div class="stat-value">${Math.max(...data.data.map((d: any) => d.high)).toFixed(4)}</div>
            <div class="stat-label">Period High</div>
          </div>
          <div class="stat-box">
            <div class="stat-value">${Math.min(...data.data.map((d: any) => d.low)).toFixed(4)}</div>
  ...
  private static generatePriceDataSection(data: ExportData): string {
    if (!data.data || data.data.length === 0) return ''
    ...
    const rows = displayData.map((candle: any) => {
  ...
  private static generateIndicatorsSection(data: ExportData): string {
  ...
  private static generateSignalsSection(data: ExportData): string {
    if (!data.signals || data.signals.length === 0) return ''

    const rows = data.signals.slice(-20).map((signal: any) => {
  ...
  private static generateNewsSection(data: ExportData): string {
    if (!data.news || data.news.length === 0) return ''

    const rows = data.news.slice(0, 10).map((item: any) => {
  ...
  static async exportChartAsPNG(chartContainer: HTMLElement, filename?: string): Promise<void> {
    try {
